...
int main() {
    ...
    vector<Texture> attack_front(FRAME_ATTACKING), attack_back(FRAME_ATTACKING), attack_left(FRAME_ATTACKING), attack_right(FRAME_ATTACKING);
    ...
    while (window.isOpen()) {
        ...
        if (attacking) {
            ...
            switch (direction) {
                case FRONT: playerSprite.setTexture(attack_front[int(CurrentFrameTime)]); break;
                case BACK:  playerSprite.setTexture(attack_back[int(CurrentFrameTime)]); break;
                case LEFT:  playerSprite.setTexture(attack_left[int(CurrentFrameTime)]); break;
                case RIGHT: playerSprite.setTexture(attack_right[int(CurrentFrameTime)]); break;
            }
        ...
        camera.update(playerSprite.getPosition().x, playerSprite.getPosition().y);
        debugOverlay.setInfo(playerSprite, currentspeed);
        ocean.draw(window);
        camera.draw(window);
        island.draw(window);
